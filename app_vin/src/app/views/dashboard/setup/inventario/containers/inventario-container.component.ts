import { Pedido } from '../models/inventario';
...
export class InventarioContainerComponent implements OnInit {
    ...
    public eventNew($event: boolean): void {
        if ($event) {
            const pedidoForm = this._matDialog.open(InventarioNewComponent);
    ...
    openModalEdit(data: Pedido) {
        console.log(data);
        if (data) {
            const pedidoForm = this._matDialog.open(InventarioEditComponent);
