        ...
        <div class="w-full mx-auto p-6 bg-white rounded overflow-hidden shadow-lg">
            ...
            <div class="flex justify-between items-center mb-2 bg-slate-300 text-black p-4 rounded">
                <h2 class="text-2xl font-bold">
                    Lista de <span class="text-primary">inventario</span>
                </h2>
                ...
                <div class="p-2 overflow-scroll px-0">
                    <table class="w-full table-fixed">
                        ...
                        <tr>
                            ...
                            <th class="w-1/6 table-header text-center border-r">Descripcion</th>
                            <th class="w-1/6 table-header text-center border-r">Stock</th>
                            <th class="w-1/6 table-header text-center border-r">Mara</th>
                            <th class="w-1/6 table-header text-center border-r">Codigo</th>
                            <th class="w-1/6 table-header text-center border-r">ProbeedorId</th>
                            <th class="w-2/6 table-header text-center">Acciones</th>
                        </tr>
                        </thead>
                        <tbody class="bg-white" *ngFor="let r of invetarios; let i = index">
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="w-2/6 p-2 text-start border-b text-sm">{{ r.estadoProducto }}</td>
                            <td class="w-2/6 p-2 text-start border-b text-sm">{{ r.marca }}</td>
                            <td class="w-2/6 p-2 text-start border-b text-sm">{{ r.codigo }}</td>
                            <td class="w-2/6 p-2 text-start border-b text-sm">{{ r.proveedorId }}</td>
                        ...
                        <tr *ngIf="isDetalleVisible[i]">
                            <td colspan="6" class="p-2 bg-gray-100">
                                <table class="w-full table-fixed border">
                                    ...
                                    <tbody>
                                    <tr *ngFor="let detalle of r.inventarioDetalle; let j = index" class="hover:bg-white">
                                        <td class="w-1/6 p-2 text-center border-b">{{ j + 1 }}</td>
                                        <td class="w-2/6 p-2 text-center border-b">{{ detalle.categoria }}</td>
...
export class InventarioListComponent implements OnInit {
    ...
    this.isDetalleVisible = new Array(this.invetarios.length).fill(false);
    }
